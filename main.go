...
func init() {
	...
	f.IntVar(&appFlags.GraphTestConf.EdgeSize, "graph-edge-size", 8192, "The size of the payload field in bytes in all vertices (graph tests)")
	...
	f.IntVar(&appFlags.GraphTestConf.BatchSize, "graph-batch-size", 250, "Batch size for creating documents (graph tests)")
	...
	f.IntVar(&appFlags.DocColConfig.BatchSize, "doc-batch-size", 250, "Batch size for creating documents in bulk mode in document collection tests")
	f.IntVar(&appFlags.DocColConfig.DocumentSize, "doc-document-size", 20480, "The size of the payload field in bytes in regular documents in document collection tests")
